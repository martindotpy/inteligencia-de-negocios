...
El dataset utilizado en este trabajo es el Brazilian E-Commerce Public Dataset by Olist, publicado en Kaggle bajo el siguiente enlace: #link("https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce")
...
#figure(
  table(
    columns: (auto, auto, auto),
    table.header[Fuente de datos][Volumen diario estimado][Volumen anual proyectado],
...
#figure(
  table(
    ...
    [58,259 registros (~58%)],
    [Análisis basado únicamente en el score numérico; NLP solo sobre comentarios existentes],
    ...
    [Media ~4.0/5.0],
    [Balanceo de clases para modelos predictivos; análisis de sentimiento ponderado],
...
#figure(
  table(
    ...
    [Procesamiento batch],
    [Apache Hadoop HDFS: Almacenamiento distribuido del histórico de datos (petabytes). Apache Spark (batch): Procesamiento de datos históricos para entrenamiento de modelos ML],
    [Procesamiento streaming],
    [Apache Spark Streaming: Procesamiento de flujos en tiempo real para detección temprana de retrasos],
    [Almacenamiento analítico], [Data Lake (HDFS/S3): Almacenamiento unificado de datos crudos y procesados],
    [Machine Learning],
    [Modelo predictivo Random Forest / XGBoost para predicción de retrasos en entregas basado en variables históricas (distancia, categoría, transportista, precio de flete, temporada)],
    [Visualización],
    [Power BI / Tableau: Dashboards interactivos con KPIs como porcentaje de entregas a tiempo, score promedio por vendedor y tasa de retraso por categoría de producto],
    table.hline(),
  ),
